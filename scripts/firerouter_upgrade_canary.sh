...
FWRCMD="curl -s --max-time 5 -H 'Authorization: Bearer $(redis-cli hget sys:ept token)' 'https://ota.firewalla.com?type=box&model=${FIREWALLA_PLATFORM}&branch=${FR_BRANCH}' | jq '. | length' "
ratio=$(eval $FWRCMD)
if [ "$ratio" == "0" ];
then
